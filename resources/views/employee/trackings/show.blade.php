                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body px-0 pt-0 pb-1">
                        <div class="table-responsive">
                            <table class="table align-items-center mb-1 py-4">
                                <thead>
                                    <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center text-xs text-secondary">
                                                    <div class="dropdown d-flex align-items-center">
                                                        <a class="btn btn-link px-3 mb-0 dropdown-toggle {{ $tracking->taskStatus->name == 'Finalizado' ? 'text-success' : 'text-dark' }}"
                                                            id="dropdownStatusOptions" data-bs-toggle="dropdown"
                                                            aria-expanded="false">
                                                            {{ $tracking->taskStatus->name }}
                                                        </a>

                                                        <ul class="dropdown-menu px-2 me-sm-n4 pb-1" aria-labelledby="dropdownStatusOptions">
                                                            <li>
                                                                <form method="POST"
                                                                    action="{{ url('/empleado/trackings/' . $tracking->id . '/edit/status') }}">
                                                                    @method('put')
                                                                    @csrf
                                                                    <input type="hidden" name="status" value="En proceso">
                                                                    <button class="dropdown-item" type="submit">En
                                                                        proceso</button>
                                                                </form>
                                                            </li>
                                                            <li>
                                                ...
                                                <td class="text-center td-actions">
                                                    <a class="btn btn-link text-info px-3 mb-0" data-bs-toggle="modal" data-bs-target="#modalAddObservation{{ $tracking->id }}">
                                                        <i class="fas fa-pencil-alt text-info me-2" aria-hidden="true"></i>
                                                        Observación
                                                    </a>
                                                </td>
                                            </tr>
                                            @include('employee/trackings/modalAddObservation', ['tracking' => $tracking])
                                            @endforeach
                                    @else
                                        <tr>
...
<script>
    ...
    window.setTimeout(function() {
        ...
        // Opcional: Eliminar el alert del DOM después de la transición
        window.setTimeout(function() {
            if (alert && alert.parentNode) {
                alert.parentNode.removeChild(alert);
            }
        }, 1000); // Tiempo suficiente para que la transición de opacidad termine
    }, 5000);
</script>
